...
struct Customer {
    ...
    int32_t c_id;          // 客户编号
    uint32_t c_d_id;        // 区域编号
    uint32_t c_w_id;        // 仓库编号
    // Other
...
struct History {
    ...
    int32_t h_c_id;         // 客户编号
    uint32_t h_c_d_id;      // 客户所在区域编号
    uint32_t h_c_w_id;      // 客户所在仓库编号
    // Foreign Key - (h_d_id, h_w_id) => (d_id, d_w_id)
    uint32_t h_d_id;        // 区域编号
    uint32_t h_w_id;        // 仓库编号
    // Other
    uint32_t h_amount;      // 支付金额
};
...
struct NewOrder {
    ...
    uint32_t no_d_id;       // 区域编号
    uint32_t no_w_id;       // 仓库编号
};
...
struct Order {
    ...
    uint64_t o_id;          // 订单编号
    uint32_t o_d_id;        // 区域编号
    uint32_t o_w_id;        // 仓库编号
    int32_t o_c_id;         // 客户编号
    // Other
    uint64_t o_entry_d;     // 订单日期
    uint32_t o_carrier_id;  // 承运人编号
...
struct OrderLine {
    ...
    uint32_t ol_d_id;       // 区域编号
    uint32_t ol_w_id;       // 仓库编号
    uint32_t ol_number;     // 订单行号
    // Foreign Key - (ol_i_id, ol_supply_w_id) => (s_i_id, s_w_id)
    uint32_t ol_i_id;       // 商品编号
    uint32_t ol_supply_w_id;// 供应仓库编号
    // Other
    uint64_t ol_delivery_d; // 交货日期
    uint32_t ol_quantity;   // 商品数量
    uint32_t ol_amount;     // 商品金额
...
struct Stock {
    ...
    uint32_t s_i_id;        // 商品编号
    uint32_t s_w_id;        // 仓库编号
    // Other
    uint32_t s_quantity;    // 商品数量
    uint64_t s_ytd;         // 商品总销售数量
    uint32_t s_order_cnt;   // 商品订单数量
